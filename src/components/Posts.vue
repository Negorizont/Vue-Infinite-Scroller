<template>
  <div class="posts" >
    <Post v-for="post of posts" :key="post.id" v-bind:post="post" />
  </div>
</template>
...
export default {
  ...
  mounted() {
    axios
      .get(`https://5e63f7a3782c970014a89ee5.mockapi.io/api/post?page=1&limit=10`)
      .then(response => {
        this.posts = response.data;
        this.posts.forEach(function(dataItem) {
          dataItem.createdAt = dataItem.createdAt.split('T')[0].split('-').reverse().join('-');
        })
      }); 
